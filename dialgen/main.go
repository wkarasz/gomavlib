...
func main() {
	...
	outFpath := kingpin.Flag("output", "output file").Required().String()
	...
	err := do(*quiet, *outFpath, *preamble, *mainDefAddr)
	if err != nil {
		fmt.Printf("Error: %s\n", err)
		os.Exit(1)
	}
...
func do(quiet bool, outFpath string, preamble string, mainDefAddr string) error {
	if strings.HasSuffix(outFpath, ".go") == false {
	...
	outDir, outFile := filepath.Split(outFpath)

	// create output folder
	os.Mkdir(outDir, 0755)

	// dump dialect.go
	err = tplDump(outFpath, tplDialect, map[string]interface{}{
		"PkgName":  pkgName,
		"Preamble": preamble,
		"Version": func() int {
	...
	outTestFile := strings.TrimSuffix(outFile, ".go") + "_test.go"
	outTestFpath := filepath.Join(outDir, outTestFile)

	// dump dialect_test.go
	err = tplDump(outTestFpath, tplTest, map[string]interface{}{
		"PkgName": pkgName,
	})
	if err != nil {
		return err
...
func tplDump(fpath string, tpl *template.Template, data map[string]interface{}) error {
	// open file
